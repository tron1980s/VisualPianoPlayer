...
Use Quick Select to pick any discovered MIDI from the app folder or `imported-midi/`. The gear button opens Song Slots, where keys `1` through `9` can be bound to songs. On first run, empty slot settings are filled from the bundled MIDI files. Slot hotkeys ignore keyboard events generated by playback, so a song pressing `1` through `9` will not switch songs by itself.
